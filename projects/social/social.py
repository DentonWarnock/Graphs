...
class SocialGraph:
    ...
    def fisher_yates(self, arr):
        ...
        for index in range(len(copy_arr)):
        # choose a random index
            rand_index = random.randint(0, len(copy_arr) -1)
        # swap the current item with that random index
            copy_arr[index], copy_arr[rand_index] = copy_arr[rand_index], copy_arr[index]
    ...
    def get_all_social_paths(self, user_id):
        ...
        Recursion --> would need to be depth first
...
if __name__ == '__main__':
    ...
    ## connections dictionary holds everyone in one's extended social network
